...
#    endif
    ...
    if (okay) {
        digitizer_set_shared_report(temp_state);
    }
    return okay;
}
...
#    endif
    ...
    split_shared_memory_lock();
    memcpy(&digitizer, &split_shmem->digitizer, sizeof(split_slave_digitizer_sync_t));
    split_shared_memory_unlock();

    digitizer.report   = digitizer_get_report();
    digitizer.checksum = crc8(&digitizer.report, sizeof(digitizer.report));
