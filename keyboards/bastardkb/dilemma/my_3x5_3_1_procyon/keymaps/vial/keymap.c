...
bool digitizer_task_kb(digitizer_t *const state) {
    static bool     last_pointer_valid = false;
    static uint16_t last_pointer_x     = 0;
    static uint16_t last_pointer_y     = 0;
    ...
    for (int i = 0; i < DIGITIZER_CONTACT_COUNT; ++i) {
        const digitizer_contact_t *contact = &state->contacts[i];
        if (contact->type != FINGER || !contact->tip || !contact->in_range) {
            continue;
        }

        if (!last_pointer_valid) {
            best_index = i;
            break;
        }

        const uint32_t dx = contact->x > last_pointer_x ? contact->x - last_pointer_x : last_pointer_x - contact->x;
        const uint32_t dy = contact->y > last_pointer_y ? contact->y - last_pointer_y : last_pointer_y - contact->y;
        const uint32_t score = (dx * dx) + (dy * dy);
    ...
    if (best_index < 0) {
        last_pointer_valid = false;
        return false;
    }

    last_pointer_valid = true;
    last_pointer_x     = state->contacts[best_index].x;
    last_pointer_y     = state->contacts[best_index].y;

    if (best_index != 0) {
        digitizer_contact_t temp     = state->contacts[0];
        state->contacts[0]           = state->contacts[best_index];
        state->contacts[best_index]  = temp;
    }
